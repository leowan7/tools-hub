...
def _contact_cell(row_html: str) -> str:
    ...
    An ALLOWLIST, not a blocklist. Two earlier drafts blocked specific
    wordings and both were escaped: first by moving the retracted claim into
    a ``<span title=...>``, then -- after that was closed -- by moving it onto
    the ``<td>`` itself, whose attributes the old regex discarded because it
    captured only the cell's inner HTML. A paraphrase using none of the
    blocked words escaped it too. So the rule is inverted: the whole cell,
    open tag included, must look like one of the sanctioned shapes and carry
    ...
    # Every attribute value in the whole cell, open tag included.
    #
    # Single-quoted raw string on purpose. A double-backslashed version of this
    # pattern matched a literal backslash, found no attributes at all, and left
    # the loop below inert while this file stayed green -- the same shape of
    # dead guard the loop exists to prevent.
    attrs = re.findall(r'\b(\w+)\s*=\s*"([^"]*)"', cell)
    for name, value in attrs:
        assert name in ("class", "style"), (
            f"contact cell carries a {name}= attribute, which can hold a claim "
            f"the label itself is not allowed to make: {cell}"
        )
        if name == "class":
            assert value in ("mono-cell",), f"unexpected class {value!r}"
