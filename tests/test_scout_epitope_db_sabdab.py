...
class TestRecallIsNotTruncatedByThePageSize:
    ...
    def test_a_full_page_with_no_hits_is_not_a_genuine_zero(self, served, monkeypatch):
        """Detection is not prevention: a truncated search must not be CACHED.

        If RCSB hands back exactly as many ids as were asked for, there may be
        more it did not send -- and if none of the ids it did send are antibody
        complexes, the honest answer is "nothing was established", not "this
        target has no known binders". Reporting [] would let
        ``fetch_known_binders`` write that into a cache with no expiry, which is
        the identical shape to the bug the page size was raised to fix.

        Not reachable today -- the largest accession measured is 3668 entries
        against a 10000 page -- which is exactly why it needs a test rather than
        a reader noticing.
        """
        served(_SUMMARY_CSV)
        monkeypatch.setattr(epitope_db, "_RCSB_ROWS_MAX", 3)
        monkeypatch.setattr(
            epitope_db, "_rcsb_pdb_ids_for_uniprot",
            lambda acc, limit=None: ["1B00", "1B01", "1B02"],   # a FULL page
        )
        assert epitope_db.query_sabdab("P0DTC2") is None, (
            "a full page of non-matching ids was reported as a genuine zero"
        )
        assert epitope_db.fetch_known_binders("P0DTC2") == []
        assert "P0DTC2" not in epitope_db._CACHE, (
            "a partial answer was cached as a permanent miss"
        )

    def test_a_short_page_with_no_hits_is_still_a_genuine_zero(self, served, monkeypatch):
        """The complement. Returning None for every empty result would stop the
        cache ever recording a real "no known binders", which is what makes the
        lookup cheap on repeat views."""
        served(_SUMMARY_CSV)
        monkeypatch.setattr(
            epitope_db, "_rcsb_pdb_ids_for_uniprot",
            lambda acc, limit=None: ["1B00"],                   # far short of the page
        )
        assert epitope_db.query_sabdab("P0DTC2") == []
        assert epitope_db.fetch_known_binders("P0DTC2") == []
