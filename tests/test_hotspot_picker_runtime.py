...
def _form_script(app, slug: str) -> str:
    """The inline picker <script> the real route ships for ``slug``."""
    ...
    blocks = [
        b for b in _SCRIPT_RE.findall(resp.get_data(as_text=True))
        if "initHotspotPicker" in b
    ]
    assert len(blocks) == 1, (
...
def _drive(script: str, *, chain="", hotspots="", clicks=()) -> dict:
    scenario = {
...
def scripts(flask_app):
    return {slug: _form_script(flask_app, slug) for slug in ALL_PICKER_FORMS}
