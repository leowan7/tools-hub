...
Each returned dict has:
    ...
    contact_residues list[int] — Antigen residue numbers at the interface.
                          ...
                          not retry it, and it arrives SIX ways:
                            1. SAbDab records no antigen chain (2347 of 22201
                               rows, 10.6%, measured 2026-09-04) or no
                               antibody chain -- no download at all;
                            2. the entry is mmCIF-only, so the .pdb fetch
                               404s -- no coordinates read;
                            3. the antigen chain is not in the legacy file;
                            4. no antibody atoms are found;
                            5. the antigen chain contributes no POLYMER
                               residues -- an all-HETATM chain (carbohydrate,
                               hapten, ion), which _parse_summary_csv can
                               select because it takes antigen_chain's first
                               component without consulting antigen_type;
                            6. the interface genuinely has nothing within the
                               cutoff.
                          Only the last CAN be a measurement, so a caller must
                          not render [] as "none found".
...
def query_sabdab(uniprot_id: str) -> Optional[list]:
    ...
    if not all_rows:
        # A FULL page is not a trustworthy zero. If RCSB returned exactly as
        # many ids as were asked for, there may be more it did not send (the
        # helper logs a warning saying so), and none of the ids it DID send
        # happened to be antibody complexes. Reporting [] here would let
        # fetch_known_binders write a permanent "no known binders" on a
        # partial answer -- the identical shape to the bug this module's page
        # size was raised to fix. ``None`` says "nothing was established",
        # which is the truth and is not cached.
        #
        # Not reachable today: the largest accession measured is 3668 entries
        # against a 10000 page. It is here so that stops mattering.
        if len(pdb_ids) >= _RCSB_ROWS_MAX:
            return None
        return []
