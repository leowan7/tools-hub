...
**Ship gate (Wave 2): 🔴 BLOCKED.** Live pilot `faa607c1` (2026-05-28) FAILED at the `ToolPayload` contract gate — `job_id` missing from `JOB_PAYLOAD` (see FAIL row above). Do NOT flip `FLAG_TOOL_RFANTIBODY=on` until the llm-proteinDesigner `run_pipeline` job_id fix lands, the app redeploys, and a fresh pilot passes. The pre-regression code-check vs `64c4ab0` was clean; the break was introduced by the 2026-05-27 21:32 repo-separation deploy.
