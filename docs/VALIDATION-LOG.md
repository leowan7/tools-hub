...
| When | Tool | Tier | Env | Commit | GPU-s | Verdict | Operator | Notes |
|---|---|---|---|---|---|---|---|---|
| 2026-04-24 | mpnn | smoke | main | `6a22b1b` | ~24 | **FAIL** | agent-a92dccc7 | First fresh smoke on `ranomics-mpnn-prod` after `modal deploy` (app deployed successfully, Layer-1 `layer1 ok` printed). Job `smoke-1777041481`. Real MPNN invocation FAIL (not stub-rejection). `protein_mpnn_run.py` exited 1 with `RuntimeError: Numpy is not available` from `protein_mpnn_utils.py:411` (`torch.from_numpy(pssm_coef_all)`). Root cause: NumPy 2.2.6 in image vs PyTorch 2.2.2+cu118 which was compiled against NumPy 1.x (warning in build log: `A module that was compiled using NumPy 1.x cannot be run in NumPy 2.2.6`). Fix: pin `numpy<2` in `tools/mpnn/Dockerfile.modal` before the MPNN step (or upgrade torch to a 2.3+ wheel that supports NumPy 2.x). Second consecutive smoke NOT attempted — first must pass first. Two-PASS streak stands at 0. |
