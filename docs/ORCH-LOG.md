...
### 2026-05-27 — contracts/ v1.0.0 bootstrap (repo-separation refactor)

- **Requested by:** orchestrator (Leo) — post-Wave-4 review remediation
- **Affects:** all streams that cross the tools-hub / llm-proteinDesigner boundary; files `contracts/__init__.py`, `contracts/rpc.py`, `contracts/upload_urls.py`, `contracts/CONTRACTS_SHA256.lock`, `.github/workflows/contracts-drift.yml` (vendored byte-identical in the sibling repo)
- **Question:** How is the request/response payload shape between tools-hub and the Modal-side pipelines pinned now that the two repos are separated?
- **Decision:** **Introduce `contracts/` as the shared boundary.** The directory holds the pydantic v2 models (`ToolPayload`, `UploadUrlsRequest`, `UploadUrlsResponse`) plus `CONTRACT_VERSION = "1.0.0"` and is vendored byte-identical into the sibling `llm-proteinDesigner` repo, which mounts it into each composite Modal image at `/opt/contracts` via `modal.Image.add_local_dir(...)`. CI verifies both copies stay in lock-step via SHA256 hashes pinned in `contracts/CONTRACTS_SHA256.lock`.
- **Rationale:** Post-separation the two repos must agree on payload shape but neither can directly import the other. Vendor-with-drift-guard keeps the contract explicit (any change must land in both repos in the same release and refresh the lockfile in both) while letting each repo own its own deploy path. The `CONTRACTS_SHA256.lock` excludes `__init__.py` because each repo carries a per-repo sync-source comment in that file.
- **Follow-up:** Wave 2 introduced `contracts/` (commit `b1a708a` — `feat(contracts): vendored RPC contract module shared between repos`). Wave 4 completed vendoring into the sibling repo. Wave 5 added the CI drift guard (commit `b438698` — `ci: add contracts SHA256 drift guard`). Any future bump of `CONTRACT_VERSION` requires a new ORCH-LOG entry plus lock-step PRs in both repos.
